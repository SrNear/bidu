...
module.exports = {
  async play(song, message) {
    ...
    if (!song) {
      ...
      return queue.textChannel.send("🚫 A fila de música acabou ;-;.").catch(console.error);
    }
    ...
    try {
      var playingMessage = await queue.textChannel.send(`🎶 Começou a reproduzir: **${song.title}** ${song.url}`);
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔁":
          ...
          queue.textChannel.send(`Loop is now ${queue.loop ? "**Ligado**" : "**Desligado**"}`).catch(console.error);
          break;
