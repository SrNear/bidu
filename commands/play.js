...
module.exports = {
  ...
  cooldown: 3,
  aliases: ["p"],
  description: "Reproduz audios do YouTube ou Soundcloud",
  async execute(message, args) {
    const { channel } = message.member.voice;

    const serverQueue = message.client.queue.get(message.guild.id);
    if (!channel) return message.reply("Você precisa estar em um canal de voz primeiro! .-.").catch(console.error);
    ...
    if (!permissions.has("CONNECT"))
      return message.reply("Não consigo conectar ao canal de voz, verifique se tenho as permissões adequadas!");
    ...
    if (urlValid) {
      ...
      try {
        ...
        console.error(error);
        return message.reply("Nenhum vídeo foi encontrado com esse título ;-;").catch(console.error);
      }
    }
    ...
    if (serverQueue) {
      ...
      return serverQueue.textChannel
        .send(`✅ **${song.title}** foi adicionado na fila por ${message.author}`)
        .catch(console.error);
    }
